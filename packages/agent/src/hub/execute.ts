...
// Every executor takes onEvent so progress flows through the SAME per-request
// callback as chunks/done/structure/error — no global/module-level state anywhere.
...
export async function executeSearchStep(
  ...
  if (findings.escalate) {
    onEvent(progressEvent("escalating"));
  ...
  while (!complete && missing.length > 0 && retryCount < MAX_SEARCH_RETRIES) {
    onEvent(progressEvent("retrying"));
    const retryFindings = await searchSpoke(
...
export async function executeAnalyzeStep(
  ...
  onEvent(progressEvent("analyzing"));
  console.log("  [hub → analyze_spoke]");
  const analysis = await analyzeSpoke(userMessage, model, signal);
...
export async function executeExplainStep(
  ...
  onEvent(progressEvent("explaining"));
  ...
  const exp = await explainSpoke(findingsToExplain, userMessage, onEvent, analysisMode, model, signal);
...
export async function executeStep(
  ...
  if (stepLower.includes("explain")) {
    return executeExplainStep(userMessage, searchFindings, analysisFindings, explanation, onEvent, model, signal);
  }

  if (stepLower.includes("analyz")) {
    return executeAnalyzeStep(userMessage, searchFindings, analysisFindings, explanation, onEvent, model, signal);
  }
